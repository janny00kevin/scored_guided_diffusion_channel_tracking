...
def main():
    ...
    plt.plot(SNR_dB_Range, rate_modal_avg, 'r--s', label=f'Modal Domain (with last $r_T={R_T}$ eigenvectors)', linewidth=2)
    ...
    # Save Filename
    filename = f"Rate_{FREQ_GHZ}GHz_{TX_DIM[0]}x{TX_DIM[1]}Tx_{RX_DIM[0]}x{RX_DIM[1]}Rx_RT_inv_{R_T}_RR{R_R}.png"
